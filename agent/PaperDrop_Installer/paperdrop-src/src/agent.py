...
class DeviceState(Enum):
    ...
    WIFI_SETUP = "wifi_setup"       # AP mode, waiting for WiFi config
        ...
        while self.running:
            try:
                if self.config.has_wifi_credentials():
                    # We have WiFi credentials - try to connect
                    success = await self.connect_to_home_wifi()
                    if success:
                        await self.run_online_mode()
                    else:
                        # WiFi connection failed - fall back to setup mode
                        logger.warning("WiFi connection failed, entering setup mode")
                        await self.run_wifi_setup_mode()
                else:
                    # No WiFi credentials - run setup mode
                    await self.run_wifi_setup_mode()
